...
class tcBillboardInvoiceGetListProcessor extends modObjectGetListProcessor
{
    public $object = 'tcBillboardOrders';
    public $classKey = 'tcBillboardOrders';
    public $defaultSortField = 'id';
    public $defaultSortDirection = 'DESC';
    /** @var  tcBillboard $tcBillboard */
    public $tcBillboard;
    public $permission = 'tborder_invoice_list';
    ...
    public function exportPdf($list, $ids)
    {
        ...
        if ($this->tcBillboard->mkdir($path, 0755)) { // Создать временную папку
            foreach ($list as $item) {
                if (in_array($item['id'], $ids) && !empty($item['invoice'])) {
                    $f = substr(strrchr($item['invoice'], '/'), 1);
                    // Копируем выбранные файлы во временную папку
                    copy(MODX_CORE_PATH . $item['invoice'], $path . $f);
                    unset($f);
                }
            }
            // Архивируем и отправляем на скачивание
            if ($zip = $this->tcBillboard->packZip('invoice')) {
            ...
            foreach ($ids as $id) {
                if ($res = $this->modx->getObject($this->classKey, (int)$id)) {
                    if (!empty($res->get('invoice'))) {
                        $res->set('invoice_downloaded', 1);
                        $res->save();
                    }
                }
            }
...
return 'tcBillboardInvoiceGetListProcessor';